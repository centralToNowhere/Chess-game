var positions = {
  ...
  set_updates: function (updates) {
    ...
    updates_container.updates = updates; // stack of commited moves

    this.moves_stack.push(updates_container);
    this.call++; // output count of moves was done in html element (positions.ai_nodes_checked_elem)
    ...
    for (var t in updates) {
      if (t === 'game_status') {
        ...
        updates_container.win = this.win; //console.log(updates.current_move);
      ...
      if (t !== 'deleted' && t !== 'current_move' && t !== 'current_side' && t !== 'name' && t !== 'password' && t !== 'is_it_a_first_move' && t !== 'game_status' && t !== 'win' && t !== 'check') {
        if (typeof updates[t] !== 'string') {
          updates_container.origin_figure = updates_container.origin_figure || {}; // stores origin figure to undo later
          ...
          for (var i in this.data) {
            if (i.match(regex)) {
              ...
              this.data[updates[t].split('_')[0] + '_' + color + '_' + ++num] = [t.split('_')[0] - 0, t.split('_')[1] - 0]; // store transform figure to delete it if undo

              updates_container.transform = updates[t].split('_')[0] + '_' + color + '_' + num; //delete pawn from spot
          ...
          if (trigger === false) {
            ...
            updates_container.transform = updates[t] + '_1'; //delete pawn from spot
    ...
    if (updates.current_move !== undefined) {
      updates_container.current_move = this.current_move; /// always true in game with AI
  ...
  get_moves: function (moves, j, k, object, side) {
    ...
    for (var i = 0; i < moves.vectors.length; i++) {
      ...
      if (moves.distance === 1) {
        for (var j = coordinates[0], k = coordinates[1]; j <= coordinates[0] + 1 && k <= coordinates[1] + 1 && j >= coordinates[0] - 1 && k >= coordinates[1] - 1 && j < 8 && k < 8 && j >= 0 && k >= 0;) {
          if (j !== coordinates[0] || k !== coordinates[1]) {
            //castling
              ...
              if (object.is_it_a_first_move.king_white) {
                if ('rook1_white' in object.data && object.data['rook1_white'][0] !== null) {
                  if (object.is_it_a_first_move.rook1_white) {
                    if (object.matrix[7][1] === null && object.matrix[7][2] === null && object.matrix[7][3] === null) {
                ...
                if ('rook2_white' in object.data && object.data['rook2_white'][0] !== null) {
                  if (object.is_it_a_first_move.rook2_white) {
                    if (object.matrix[7][5] === null && object.matrix[7][6] === null) {
              ...
              if (object.is_it_a_first_move.king_black) {
                if ('rook1_black' in object.data && object.data['rook1_black'][0] !== null) {
                  if (object.is_it_a_first_move.rook1_black) {
                    if (object.matrix[0][1] === null && object.matrix[0][2] === null && object.matrix[0][3] === null) {
                ...
                if ('rook2_black' in object.data && object.data['rook2_black'][0] !== null) {
                  if (object.is_it_a_first_move.rook2_black) {
                    if (object.matrix[0][5] === null && object.matrix[0][6] === null) {
      ...
      if (moves.distance === -1 || moves.distance === 2) {
        if (coordinates[0] + moves.vectors[i][0] < 8 && coordinates[1] + moves.vectors[i][1] < 8 && coordinates[1] + moves.vectors[i][1] >= 0 && coordinates[0] + moves.vectors[i][0] >= 0) {
          ...
          if (object.matrix[coordinates[0] + moves.vectors[i][0]][coordinates[1] + moves.vectors[i][1]] === null) {
  ...
  execute_move: function (start_j, start_k, finish_j, finish_k, embeded_move, object) {
    ...
    function callback(e) {
      ...
      if (start_j !== undefined && start_k !== undefined && finish_k !== undefined && finish_j !== undefined) {
        ...
        updates[start_j + '_' + start_k] = [finish_j, finish_k]; //	object.board_cells[start_j*8 + start_k].className = object.board_cells[start_j*8 + start_k].className.replace(' ' + object.matrix[start_j][start_k], "");
        ...
        if (typeof embeded_move === 'object') {
          if (embeded_move[0] === 'castling') {
            ...
            updates.deleted.push([embeded_move[1], embeded_move[2]]); //	object.board_cells[embeded_move[1]*8 + embeded_move[2]].className = object.board_cells[embeded_move[1]*8 + embeded_move[2]].className.replace(' ' + object.matrix[embeded_move[1]][embeded_move[2]], "");
          ...
          backlight_move: function (e) {
            ...
            return function () {
              ...
              if (this.current_move === true && this.matrix[j][k] !== null && this.matrix[j][k].split('_')[1] === this.current_side) {
                ...
                var res = this.get_moves(this.moves[figure](), j, k, object); //set callbacks on each of the possible cells for move

                var arr = res.possible_attacks.concat(res.possible_cells); //scan check or stalemate
                //delete moves from arr to prevent king's vanishing

                var to_delete = this.tools.scanCheckmate(); //console.log(to_delete);
                ...
                var all_moves_side = this.tools.moves_intersection(undefined, [], this.data, undefined, undefined, 'true'); //console.log('COMPARE', all_moves_side, to_delete);
                ...
                if (all_moves_side_length === 0 && this.check === this.current_side) {
                  ...
                  this.game_status = 'end'; ///execute move only to send information about checkmate
                ...
                if (all_moves_side_length === 0 && this.check === '') {
                  this.win = 'stalemate';
                  this.game_status = 'end'; ///execute move only to send information about stalemate
                ...
                if (this.win === "") {
                  ...
                  for (var i = 0; i < arr.length; i++) {
                    this.board_cells[arr[i][0] * 8 + arr[i][1]].addEventListener('click', this.execute_move(j, k, arr[i][0], arr[i][1], arr[i][2], this), false);
                  }
  ...
  tools: {
    ...
    moves_intersection: function (moves_for_delete, arr, object, reg) {
      ...
      var intersection = [];
      reg = arguments[3] === undefined ? new RegExp('.+_' + positions.current_side) : arguments[3];
      ...
      for (var t in object) {
        ...
        if (t.match(reg) && king && positions.data[t][0] !== null) {
          var f = t.split('_')[0];
          ...
          var all_moves = positions.get_moves(positions.moves[f](), positions.data[t][0], positions.data[t][1], positions, t.split('_')[1]);
    ...
    scanCheckmate: function () {
      var op_side = positions.current_side === 'black' ? 'white' : 'black';
      var side = positions.current_side;
      var reg1 = new RegExp('.+_' + op_side);
      var reg2 = new RegExp('.+_' + side);
      var moves_for_delete = {};
      moves_for_delete['king' + '_' + positions.current_side] = [];
      ...
      var fn = positions.tools.moves_intersection.curry(moves_for_delete);
      var king = positions.data['king_' + positions.current_side];
      var king_nearist_cells = [[king[0] + 1, king[1] + 1], [king[0] - 1, king[1] - 1], [king[0] + 1, king[1]], [king[0] - 1, king[1]], [king[0], king[1] + 1], [king[0], king[1] - 1], [king[0] + 1, king[1] - 1], [king[0] - 1, king[1] + 1]];

      for (var h in positions.data) {
        /// possibility of castling
        if (h === 'king_' + side) {
          if (positions.is_it_a_first_move[h] === true) {
            if (side === 'white') {
            ...
            var rt = fn(cells, positions.data, reg1, 'intersection');
        ...
        if (h.match(reg1) && positions.data[h][0] !== null && positions.data[h][1] !== null) {
          var figure = h.split('_')[0];
          var j = positions.data[h][0];
          var k = positions.data[h][1];
          figure = figure[figure.length - 1].match(/[0-9]/) != null ? figure.substr(0, figure.length - 1) : figure;
          ...
          var temp = positions.moves[figure]().vectors;
            ...
            temp.forEach(function (move) {
              king_nearist_cells.forEach(function (cell) {
                if (j + move[0] === cell[0] && k + move[1] === cell[1]) {
                  moves_for_delete['king' + '_' + positions.current_side].push(cell);
                } /// if check by knight


                if (j + move[0] === king[0] && k + move[1] === king[1]) {
                  positions.check = positions.current_side;
                  positions.set_game_status();
                  moves_for_delete = fn([[j, k]], positions.data);
                }
              }, positions);
            }, positions);
          }
          ...
          if (figure === 'bishop' || figure === 'rook' || figure === 'queen') {
            for (var z = 0; z < temp.length; z++) {
              ...
              for (var l = move[0], m = move[1], j = positions.data[h][0] + l, k = positions.data[h][1] + m; j < 8 && k < 8 && j >= 0 && k >= 0; j = j + l, k = k + m) {
                if (cells.length === 0) {
                  cells.push([j - l, k - m]);
                }
                ...
                for (var w = 0; w < king_nearist_cells.length; w++) {
                  ...
                  if (cell[0] === j && cell[1] === k || king[0] === j && king[1] === k) {
                    ...
                    if (buf.length === 0) {
                      moves_for_delete['king' + '_' + positions.current_side].push([j, k]); /// if check

                      if (king[0] === j + l && king[1] === k + m || king[0] === j && king[1] === k) {
                        positions.check = positions.current_side;
                        positions.set_game_status();
                        moves_for_delete = fn(cells, positions.data);
                      }
                    } //if one figure
                    ...
                    if (buf.length === 1) {
                      if (buf[0].split('_')[1] === op_side) {
                        king_nearist_cells.forEach(function (cell) {
                          ...
                          if (cell[0] === positions.data[buf[0]][0] && cell[1] === positions.data[buf[0]][1]) {
                            moves_for_delete['king' + '_' + positions.current_side].push(cell);
                          }
                        }, positions);
                      }
    ...
    undo: function () {
      ...
      for (var t in last_update.updates) {
        if (t === 'game_status') {
          if (last_update.game_status !== undefined) {
            positions.game_status = last_update.game_status;
          }

          if (last_update.win !== undefined) {
            positions.win = last_update.win;
          }
        }
        ...
        if (t !== 'deleted' && t !== 'current_move' && t !== 'current_side' && t !== 'name' && t !== 'password' && t !== 'is_it_a_first_move' && t !== 'game_status' && t !== 'win' && t !== 'check') {
          if (typeof last_update.updates[t] !== 'string') {
            var origin = last_update.origin_figure[t],
                m = last_update.updates[t]; // stores origin figure to undo later

            positions.matrix[t.split('_')[0]][t.split('_')[1]] = origin.split('_').length >= 3 ? origin.split('_')[0] + '_' + origin.split('_')[1] : origin;
            positions.data[origin] = [t.split('_')[0] - 0, t.split('_')[1] - 0]; // if(dataOrigin){
            // 	positions.data[dataOrigin] = [t.split('_')[0] - 0, t.split('_')[1] - 0];
            // }

            if (last_update.cut !== undefined) {
              if (last_update.cut.match(/^.+_.+_\d+/)) {
                positions.matrix[last_update.updates[t][0]][last_update.updates[t][1]] = last_update.cut.split('_')[0] + '_' + last_update.cut.split('_')[1];
              } else {
                positions.matrix[last_update.updates[t][0]][last_update.updates[t][1]] = last_update.cut;
              }

              positions.data[last_update.cut] = [last_update.updates[t][0] - 0, last_update.updates[t][1] - 0];
            } else {
              positions.matrix[last_update.updates[t][0]][last_update.updates[t][1]] = null;
            }
          } else {
            if (last_update.transform !== undefined) {
              delete positions.data[last_update.transform];
            }
          }
        }
      }
      ...
      if (typeof last_update.side !== 'undefined') {
        positions.current_side = last_update.side;
      }

      if (typeof last_update.is_it_a_first_move !== 'undefined') {
        positions.is_it_a_first_move = last_update.is_it_a_first_move;
      }

      if (typeof last_update.current_move !== 'undefined') {
        positions.current_move = last_update.current_move;
      }
    ...
    ai_output_nodes_checked: function (call) {
      ...
      if (typeof window !== 'undefined' && typeof window.requestAnimationFrame !== 'undefined') {
        requestAnimationFrame(function (time) {
          positions.ai_nodes_checked_elem.innerText = '' + call;
        });
      }
    },
  ...
  subscribe: function () {
    ...
    xhr.onload = function () {
      ...
      document.body.click(); //document.querySelector('.chat-container').dispatchEvent(e);
    };
  ...
  */
  ...
  matrix: [['rook1_black', 'knight1_black', 'bishop1_black', 'queen_black', 'king_black', 'bishop2_black', 'knight2_black', 'rook2_black'], ['pawn1_black', 'pawn2_black', 'pawn3_black', 'pawn4_black', 'pawn5_black', 'pawn6_black', 'pawn7_black', 'pawn8_black'], [null, null, null, null, null, null, null, null], [null, null, null, null, null, null, null, null], [null, null, null, null, null, null, null, null], [null, null, null, null, null, null, null, null], ['pawn1_white', 'pawn2_white', 'pawn3_white', 'pawn4_white', 'pawn5_white', 'pawn6_white', 'pawn7_white', 'pawn8_white'], ['rook1_white', 'knight1_white', 'bishop1_white', 'queen_white', 'king_white', 'bishop2_white', 'knight2_white', 'rook2_white']],
  moves: {
    king: function () {
      // moves 
