...
(function () {
  //radio buttons
  var rads = document.querySelector('.game-auth').mode; // checked initialy
          ...
          if (this.responseText === 'data-error' || this.responseText === 'Unauthorized') {
            ...
            try {
              ...
              loadScript('js/chat.js', function () {
                ...
                positions.subscribe();
                positions.render();
                positions.set_possible_moves(); // var e = new Event('click_king');
      ...
      (function () {
        ...
        loadScript('js/chessBot.js', function () {
          ...
          var bot = new ChessBot(positions),
              workerInProgress = false; // creates ui for AI
          ...
          container__pruning.appendChild(pruning_div); //wrapper for scrollbar (overflow-x: auto/scroll/hidden; overflow-y:visible problem)
          ...
          console.log('ChessBot loaded'); // var aiTurn = 0;
          ...
          document.body.addEventListener('AI_turn', function () {
            ...
            algorithm = document.querySelector('input[name="check_algo"]:checked').value; //reset metrics
              ...
              worker.addEventListener('message', function (e) {
                ...
                if (subject === 'status') {
                  ...
                  if (property === 'finished') {
                    ...
                    forward_arrow.classList.remove('active'); // check if game is already finished (if mate)
          ...
          metrics.addEventListener('click', metrics_func); // set arrow keys
          ...
          document.querySelector('.container-fluid ').style.display = 'block';
          document.querySelector('.modal').style.display = 'none'; //custom select box
          ...
          positions.render();
          positions.set_possible_moves(); //bad hack
    ...
    chat_subscribe: function (id, game) {
      ...
      xhr.onload = function () {
        try {
          ...
          var whose = data.whose; // var chatBox = new MessageBox();
