var ChessBot = function () {
  ...
  return function (object) {
    ...
    this.getMedianBranchingFactor = function (sameDepthLevelNodes) {
      sameDepthLevelNodes.sort(function (a, b) {
        return a - b;
      });
        ...
        return function (a, b) {
          ...
          var getMovesAmount = function (c) {
            var type = filter(c),
                ...
                length = 0; //make virtual move

            object.matrix[c[0][0]][c[0][1]] = null;
            object.matrix[c[1][0]][c[1][1]] = pieceFromMatrix;
            object.data[pieceFromData] = [c[1][0], c[1][1]]; // get number of attack moves

            length = object.get_moves(object.moves[type](), c[1][0], c[1][1], object).possible_attacks.length; // undo virtual move
    ...
    this.evaluate = function (eval_obj) {
      ...
      var costTable = {
        pawn: [[0, 0, 0, 0, 0, 0, 0, 0], [99, 99, 99, 99, 99, 99, 99, 99], [50, 50, 50, 50, 50, 50, 50, 50], [5, 5, 10, 25, 25, 10, 5, 5], [0, 0, 0, 20, 20, 0, 0, 0], [5, -5, -10, 0, 0, -10, -5, 5], [5, 10, 10, -20, -20, 10, 10, 5], [0, 0, 0, 0, 0, 0, 0, 0]],
        ...
        king: [[-30, -40, -40, -50, -50, -40, -40, -30], [-30, -40, -40, -50, -50, -40, -40, -30], [-30, -40, -40, -50, -50, -40, -40, -30], [-30, -40, -40, -50, -50, -40, -40, -30], [-20, -30, -30, -40, -40, -30, -30, -20], [-10, -20, -20, -20, -20, -20, -20, -10], [20, 20, 0, 0, 0, 0, 20, 20], [0, 30, 10, 0, 0, 10, 30, 0]]
      };
      var max_moves = {
        'pawn': 4,
        'knight': 8,
      ...
      for (var t in object.data) {
        i = object.data[t][0];
        j = object.data[t][1];
        piece_type = t.split('_')[0]; // t.split('_')[1] case - queen_white_2
    ...
    this.sendResult = function (tree, branchingFactorObject) {
      if (self.isWorker) {
        ...
        self.postMessage(['positions', 'ai_side', '']); // show remaining nodes amount on last tick

        postNodes(undefined, 0, true);
        progressBar(0, 0, true);
        postPruning(0, 0, true, branchingFactorObject); // 50 ms to show last tick on progressbar, then hide it
    ...
    this.negascout = function (ai_settings) {
      ...
      var branchingFactorObject = {}; //init tree

      object.AI = true;
      var real = object;
      object = this.cloner.clone(object); // now in object methods var 'positions' point to real object. We need to set object self context. So, inside object do
      //  'positions = this'

      object.setSelfContext(); //DOM element is not copied???
      ...
      var tmp_branch = 0;
      var initial_node = {
        0: [Number.NEGATIVE_INFINITY, Number.NEGATIVE_INFINITY, Number.POSITIVE_INFINITY],
        // supposed to persist array [node_value, alpha_value, beta_value, m]
        1: {},
        // link to all child objects, -1 - link to self object, -2 - child node id that leads to the best move
        2: 0,
        // link to parent object
        3: 'alpha',
        // alpha or beta node
        4: [],
        // array of move to reach this node
      ...
      var tree = branch = initial_node[1],
          ...
          move = [],
          research = false,
          researchDepthMarker = -1; //build nodes (branch  - child nodes) 

      var half_move = function half_move(research_id) {
        object.current_move = 'true';
        var arr = this.all_moves_side_func(object.current_side); // ordering
        // arr = this.orderingStrict(arr, branch[-1][3]);

        arr = this.ordering(arr); // branchingFactorObject[current_depth] = branchingFactorObject[current_depth] || [];
        ...
        for (var i = 0; i < arr.length; i++) {
          ...
          ;
          node = branch[-1][3] === 'alpha' ? 'beta' : 'alpha'; //set window
          ...
          branch[i] = {
            0: values,
            // supposed to persist array [node_value, alpha_value, beta_value]
            1: {},
            // branch of next moves after current
            2: parent_node,
            // link to parent object
            3: node,
            // alpha or beta node
            ...
            // array of move to reach this node
            5: search_window,
            // search window, 0 - null window, 1 - full search
            6: fraction
          };
          ...
          if (current_depth < depth) {
            object.current_side = object.current_side === 'white' ? 'black' : 'white';
            half_move.apply(this); // if(typeof branchingFactorObject[current_depth+1] !== 'undefined' ){
            // 	branchingFactorObject[current_depth+1].push('');
            // }

            object.current_side = object.current_side === 'white' ? 'black' : 'white'; // branch there is not an empty node
            ...
            if (branch[3] === 'alpha') {
              if (null_search === 0) {
                ...
                if (i !== 0 && res > branch[0][1] && res < branch[0][2] || (res === Number.POSITIVE_INFINITY || res === Number.NEGATIVE_INFINITY) && (branch[0][1] === res || branch[0][2] === res)) {
                  ...
                  branch[1][i] = null; // object.current_side = object.current_side === 'white' ? 'black' : 'white';
                  ...
                  if (researchDepthMarker === current_depth) {
                    researchDepthMarker = -1; //re-search ends
              ...
              if (null_search === 0) {
                ...
                if (i !== 0 && res > branch[0][1] && res < branch[0][2] || (res === Number.POSITIVE_INFINITY || res === Number.NEGATIVE_INFINITY) && (branch[0][1] === res || branch[0][2] === res)) {
                  ...
                  current_depth--;
                  trig = 1;
                  half_move.apply(this, [i]); // object.current_side = object.current_side === 'white' ? 'black' : 'white';
            ...
            res = this.evaluate(eval_obj); // branch there is a container for nodes
            // {
            //      ... empty ...
            //      -1:  --- parent node
            // }
      ...
      real.call = object.call; // restore positions object

      object = real; // return back context to real positions

      object.setSelfContext(); // real move
    ...
    */
    ...
    this.alphaBeta = function (ai_settings) {
      ...
      var res = 0;
      var calls = 0;
      var move = []; // branch here is an inner node
      // {
      //      ... empty ...
      //      -1:  --- self
      // }
      ...
      var half_move = function half_move() {
        ...
        for (var i = 0; i < arr.length; i++) {
          // fraction = parent fraction / arr.length
          var fraction = branch[-1][5] / arr.length; // move coordinates
          ...
          if (current_depth < depth) {
            object.current_side = object.current_side === 'white' ? 'black' : 'white'; // branch before
            // {
            //      ... empty ...
            //      -1:  --- self
            // }
            ...
            branch = tmp_branch[-1][2]; // inner node
            // {
            //      0: {}
            //		1: {}
            //		2: {}
